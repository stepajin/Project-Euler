...
func value(_ card: Character) -> Int {
    switch card {
        case "A": return 14
        case "K": return 13
        case "Q": return 12
        case "J": return 11
        case "T": return 10
        default: return Int(card.asciiValue!) - 48
    }
}

func value(_ card: Card) -> Int {
    value(card.type)
}

func frequencies(_ cards: [Card]) -> [Character: Int] {
    [Character: [Card]](grouping: cards, by: { $0.type }).mapValues { $0.count }
}

typealias Card = (type: Character, suit: Character)
typealias Rule = ([Card]) -> Int?

let highCard: Rule = { cards in
    cards.map(value(_:)).max()
}

let onePair: Rule = { cards in
    frequencies(cards).filter { $0.value >= 2 }.keys.map(value(_:)).max()
}

let twoPairs: Rule = { cards in
    let types = frequencies(cards).filter { $0.value >= 2 }.keys.map(value(_:))
    return types.count >= 2 ? types.max() : nil
}

let threeOfKind: Rule = { cards in
    frequencies(cards).filter { $0.value >= 3 }.keys.map(value(_:)).max()
}

let straight: Rule = { cards in
    let values = cards.map(value(_:)).sorted()
    guard values.indices.dropFirst().allSatisfy({ idx in
        values[idx] == values[idx-1] + 1
            || (idx == cards.count-1 && values[idx] == 14 && values[0] == 2)
    }) else { return nil }
    return values.last
}

let flush: Rule = { cards in
    guard Set(cards.map { $0.suit }).count == 1 else { return nil }
    return cards.map(value(_:)).max()
}

let fullHouse: Rule = { cards in
    guard frequencies(cards).values.sorted() == [2, 3] else { return nil }
    return cards.map(value(_:)).max()
}

let fourOfKind: Rule = { cards in
    frequencies(cards).filter { $0.value >= 4 }.keys.map(value(_:)).max()
}

let straightFlush: Rule = { cards in
    straight(cards) != nil ? flush(cards) : nil
}

let royalFlush: Rule = { cards in
    straightFlush(cards) == 14 ? 14 : nil
}
...
func readCard(_ raw: String) -> Card {
    Card(type: raw.first!, suit: raw.last!)
}

func readHands(_ row: Substring) -> ([Card], [Card]) {
    let cards = row.split(separator: " ").map(String.init).map(readCard(_:))
    return (Array(cards[0...4]), Array(cards[5...9]))
}

func p1Wins(_ hand1: [Card], _ hand2: [Card]) -> Bool {
    for rule in rules {
        let p1 = rule(hand1)
        let p2 = rule(hand2)
        if p1 == nil && p2 == nil { continue }
        guard let p1 = p1 else { return false }
        guard let p2 = p2 else { return true }
        return p1 > p2 || (p1 == p2 && highCard(hand1)! > highCard(hand2)!)
    }
    return false
}

let result = input.split(separator: "\n").lazy.map(readHands(_:)).filter { p1Wins($0, $1) }.count
print(result)
